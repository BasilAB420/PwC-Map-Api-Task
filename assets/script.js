...
function addSuggestions(){
  if (city.length) {
    ...
    city.map(city => {
      suggested.innerHTML += `<p class='suggested-word' data-longitude="${city.longitude}" data-latitude="${city.latitude}">${city.name}-${city.state}</p>`;
    })
...
function mapBuilder(){
  ...
  map.innerHTML = `<iframe style="width: 80%" height="200" id="gmap_canvas"
...
async function fetchGeoCoding(){
  ...
  await fetch(url, {
    ...
    headers: {
      ...
      "X-Api-Key": "+GYh/iqV/zrizZ/Cr8rH5w==SglihYfYhQUuQsfT"
    },
...
window.addEventListener("loader", () => {
  ...
  loader.classList.add("transitionend",  () => {
